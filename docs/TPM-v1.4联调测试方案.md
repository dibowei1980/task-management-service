...
| T13 | 任务类型注册审批 | 后端 + 前端 |
| T14 | 回传字段配置 | 后端 + 前端 |
| T15 | resultViewUrl 模板替换 | 后端 |
| T16 | 子任务总数上报（totalSubTaskCount） | 后端 |
...
| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T13.1.1 ✅ | BRS 提交注册申请 | BRS 启动时向 TMS POST /api/task-type-registrations | 申请状态=PENDING，包含 typeCode、typeName、callbackPath、resultViewUrl、callbackFields、resultQueryPath |
| T13.1.2 ✅ | 管理员审批通过 | system:admin 用户 POST approve，选择分组 | 创建 TaskTypeDefinition，callbackFields/resultQueryPath 同步到 ExternalSystemRegistration |
| T13.1.3 ✅ | 管理员审批拒绝 | system:admin 用户 POST reject，填写原因 | 申请状态=REJECTED，拒绝原因记录 |
| T13.1.4 | 前端审批页面 | 在类型审批页面查看待审批列表 | 显示申请详情，包含声明的回传字段摘要 |

#### T13.2 边界条件

| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T13.2.1 ✅ | 类型代码查重 | 提交已存在的 typeCode | 返回拒绝原因：类型代码已存在 |
| T13.2.2 | 非管理员审批 | 非 system:admin 用户尝试审批 | 返回 403 Forbidden |
| T13.2.3 | 审批拒绝无原因 | POST reject，remark 为空 | 返回 400 Bad Request：拒绝原因必填 |
| T13.2.4 | BRS 类型不在 H2SchemaBootstrap | 检查启动时自动注册逻辑 | BRIDGE_REMOVAL_BATCH 不在数据库中；BRIDGE_REMOVAL_UNIT 为 BRS 内部概念，不向 TMS 注册 |

#### T13.3 异常场景

| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T13.3.1 | TMS 不可用时 BRS 提交申请 | BRS 启动但 TMS 服务不可用 | 仅 warning 不阻塞，BRS 仍可本地运行 |
| T13.3.2 | 审批通过后 TMS 重启 | 审批通过后重启 TMS | TaskTypeDefinition 和 ExternalSystemRegistration 数据持久化，重启后仍可查到 |
...
| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T14.1.1 ✅ | 申请时声明回传字段 | 提交注册申请携带 callbackFields=["TASK_ID","STATUS","NAME","OPERATOR","WORKLOAD","UNIT","START_TIME","LOCATION"] 和 resultQueryPath="/api/v1/projects/{id}/result" | callbackFields JSON 序列化保存，resultQueryPath 保存 |
| T14.1.2 ✅ | 管理员调整字段配置 | PUT /{id}/callback-fields，选择子集 | 更新后的字段列表同步到 ExternalSystemRegistration |
| T14.1.3 ✅ | 下发任务传递字段 | TMS 下发任务到外部系统 | payload 包含 callback_fields 列表 |
| T14.1.4 | 前端字段配置弹窗 | 管理员在审批页面点击"字段配置" | 显示 CallbackFieldsModal，可勾选字段 |

#### T14.2 边界条件

| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T14.2.1 ✅ | 必选字段不可取消 | 管理员尝试取消 TASK_ID | 必选字段始终包含，不可取消 |
| T14.2.2 | 空字段列表 | PUT /{id}/callback-fields，callbackFields=[] | 仅保留必选字段（TASK_ID/STATUS/NAME/OPERATOR/WORKLOAD/UNIT） |
| T14.2.3 | 全量字段 | PUT /{id}/callback-fields，包含全部 10 个字段 | 全部字段保存并同步 |

#### T14.3 异常场景

| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T14.3.1 | 未知字段枚举值 | callbackFields 包含 "UNKNOWN_FIELD" | 忽略未知字段，仅保存有效枚举值 |
| T14.3.2 | 注册申请不存在 | PUT /{随机UUID}/callback-fields | 返回 404 NotFound |

### T15 resultViewUrl 模板替换

#### T15.1 正常流程

| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T15.1.1 ✅ | 注册时声明 resultViewUrl | BRS 注册时传入 resultViewUrl="http://localhost:5174/tasks/{id}/locate?tab=result" | ExternalSystemRegistration.resultViewUrl 保存 |
| T15.1.2 ✅ | 下发任务时替换 | TMS 下发任务到 BRS，任务 ID=abc-123 | task.externalUrl="http://localhost:5174/tasks/abc-123/locate?tab=result" |

#### T15.2 边界条件

| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T15.2.1 | resultViewUrl 无占位符 | 传入 resultViewUrl="http://localhost:5174/dashboard" | 直接设置 task.externalUrl，不做替换 |
| T15.2.2 | resultViewUrl 为空 | 不传入 resultViewUrl | task.externalUrl 不设置，保持 null |

### T16 子任务总数上报（totalSubTaskCount）

#### T16.1 正常流程

| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T16.1.1 | BRS 回调携带 totalSubTaskCount | PATCH /tasks/{id}/workflow-status，body 含 totalSubTaskCount=10 | task.workload=10.0，completion_data 包含 totalSubTaskCount |
| T16.1.2 | BRS 仅注册 BATCH | BRS 启动向 TMS 提交注册申请 | 仅提交 BRIDGE_REMOVAL_BATCH，不提交 BRIDGE_REMOVAL_UNIT |

#### T16.2 边界条件

| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|---------|
| T16.2.1 | totalSubTaskCount 为 0 | PATCH /tasks/{id}/workflow-status，totalSubTaskCount=0 | task.workload=0.0 |
| T16.2.2 | totalSubTaskCount 为 null | PATCH /tasks/{id}/workflow-status，不传 totalSubTaskCount | workload 不更新 |
| T16.2.3 | totalSubTaskCount 与 completedWorkload 同时存在 | 两者均传入 | totalSubTaskCount 覆盖 workload（后处理优先） |
