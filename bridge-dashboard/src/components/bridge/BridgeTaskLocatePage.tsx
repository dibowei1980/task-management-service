...
type Tfw = { a: number; b: number; d: number; e: number; c: number; f: number } | null;
...
type PreprocessSegmentsResponse = {
  taskId: string;
...
const DISPLAY_TOGGLE_STORAGE_KEY = 'bridge_task_locate_display_toggles';
const MASK_UI_STORAGE_KEY = 'bridge_task_locate_mask_ui';

const loadDisplayTogglePrefs = () => {
  try {
...
const normalizePath = (p: string | null | undefined) => (p || '').replace(/\\/g, '/').trim().toLowerCase();
...
const buildMaskCutPath = (jsonPath: string, segmentName?: string) => {
  const normalized = (jsonPath || '').replace(/\\/g, '/');
  if (!normalized) return '';
  const idx = normalized.lastIndexOf('/');
  if (idx < 0) return '';
...
const isTiffPath = (p: string) => {
  const s = (p || '').toLowerCase();
  return s.endsWith('.tif') || s.endsWith('.tiff');
};

const decodeTiffToRgba = async (buf: ArrayBuffer): Promise<Raster> => {
...
const blobUrlFromArrayBuffer = (buf: ArrayBuffer, contentType: string) => {
...
const computeBoundsWorld = (tfw: Tfw | undefined, width: number, height: number) => {
  if (!tfw) return null;
...
const pixelToWorld = (tfw: Tfw | undefined, px: [number, number]) => {
  if (!tfw) return null;
  const [col, row] = px;
  const x = tfw.a * col + tfw.b * row + tfw.c;
  const y = tfw.d * col + tfw.e * row + tfw.f;
...
const worldToPixel = (tfw: Tfw | undefined, world: [number, number]) => {
  if (!tfw) return null;
...
const pointInPolygon = (point: [number, number], poly: Array<[number, number]>) => {
  let inside = false;
...
const createBitmapFromRgba = async (raster: Raster) => {
...
const createBitmapFromImageBuffer = async (buf: ArrayBuffer, contentType: string) => {
...
const fitToBounds = (bounds: { minX: number; minY: number; maxX: number; maxY: number }, viewW: number, viewH: number): ViewTransform => {
