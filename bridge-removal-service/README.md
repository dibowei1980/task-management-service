...
- SSO 统一身份认证登录（与 TPM 相同登录方式）
- UPM 用户/部门代理
- 本地项目提交到 TMS

**任务层级**：BRS 只支持两层任务结构（项目 → 子任务单元），不涉及 PHASE 等中间层级。多层级任务结构和进度逐层汇聚由 TMS 管理。

**可独立运行**：task-management-service 不可用时，BRS 自动进入本地项目模式，任务上报/接收降级，桥梁去除核心功能不受影响，权限控制仍由 SSO/TPM 提供。
...
| `TMS_RETRY_INTERVAL` | TMS 注册重试间隔（秒） | `30` |
...
| GET | `/api/auth/sso/auth-url` | 获取 SSO 授权 URL |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/health` | 健康检查（含 TMS/SSO 连接状态） |
| GET | `/api/system/status` | TMS / UPM / SSO 连接状态（含 `local_mode`、`tms_registered`） |

## 独立运行与容错

### 认证方式

BRS 采用与 TPM 相同的登录方式，仅支持 SSO 统一身份认证或直连 TPM 登录，不支持本地帐号登录方式。所有用户必须通过 SSO 认证后才能使用系统。

### TMS 自动注册与本地项目模式

BRS 启动时自动连接并注册到 TMS：
1. 启动时执行 `register_with_task_management()`，向 TMS 发送注册请求
2. 注册成功 → 进入**联机模式**，任务上报/接收/回调正常工作
3. 注册失败 → 进入**本地项目模式**：
   - 权限控制仍由 SSO/TPM 提供（SSO 必须可用）
   - 桥梁去除项目可独立创建和执行
   - 任务上报/接收功能不可用
   - 后台线程每 30 秒（`TMS_RETRY_INTERVAL` 可配置）自动重试注册
   - TMS 恢复后自动切换回联机模式
...
| 依赖服务 | 不可用时的影响 | 容错策略 |
|----------|---------------|---------|
| task-management-service | 任务上报/接收不可用，进入本地项目模式 | 启动注册非阻塞；后台自动重连；状态回调跳过；本地项目可独立创建和执行 |
| UPM (user-service) | 用户/部门列表为空 | UPM 代理返回错误，明确告知上游服务不可达；前端显示连接异常提示 |
| SSO 服务 | SSO 统一登录不可用 | **无法登录**（不支持本地帐号）；需等待 SSO 恢复后登录 |
...
- 前端显示"需提交TMS"提示
- TMS 恢复后，有 `project:create` 权限的 SSO 用户可将本地项目提交 TMS
- 提交后质检由 TMS 侧有 `quality:check` 权限的用户完成

## 与 Task Management Service 集成

bridge-removal-service 启动时自动向 task-management-service 注册（`POST /api/external-systems/register`），注册失败不阻塞启动，系统进入本地项目模式。后台线程按 `TMS_RETRY_INTERVAL`（默认 30 秒）间隔自动重试注册，TMS 恢复后自动切换回联机模式。
