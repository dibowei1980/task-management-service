...
def _skip_if_no_tms(default=None):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(api_url, headers, *args, **kwargs):
            if not api_url:
                return default
            return func(api_url, headers, *args, **kwargs)
        return wrapper
    return decorator
...
def _get_user_management_config():
    api_url = os.getenv("USER_MANAGEMENT_API_URL") or "http://127.0.0.1:8081"
